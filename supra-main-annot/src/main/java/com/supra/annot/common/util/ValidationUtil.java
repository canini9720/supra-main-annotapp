...
public class ValidationUtil {
	...
	public static boolean isStringValueNotNullAndNotEmpty(Object strVal){
		...
		if(strVal != null && 
				(!strVal.toString().isEmpty() &&
						!CommonConstants.STR_QUESTION_MARK.equals(strVal.toString())
						)){
     ...
     * @throws Exception
     */
    public static String validateStringValue(boolean required,int minLength,int maxLength,String valStr,String exceptionCode) throws Exception{
        valStr = CommonUtil.getDefaultIfNull(valStr,null);
        if(required && valStr == null){ExceptionUtil.throwException(exceptionCode, ExceptionUtil.EXCEPTION_VALIDATION);}
    ...
    public static boolean validatePhoneNo(String strVal){
    	...
    	if(strVal != null && !strVal.isEmpty()){
    		...
    		if(isNumberOnly(strVal)){
	    		...
	    		if(strVal.matches(PHONE_UAE_PATTERN)
	    				|| strVal.matches(PHONE_OMAN_PATTERN)
	    				|| strVal.matches(PHONE_SAUDI_PATTERN)
	    				|| strVal.matches(PHONE_QATAR_PATTERN)
	    				|| strVal.matches(PHONE_KUWAIT_PATTERN)
	    				|| strVal.matches(PHONE_BAHRAIN_PATTERN)){
